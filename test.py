...
while True:
    ...
    # thresh = cv2.threshold(gray, 95, 255, cv2.THRESH_BINARY_INV)[1]
    ...
    transformedEdges = goboardImageProcessing.canny_edge(transformedGray)
    ...
    if transformedLines is not None:    
        h_lines, v_lines = goboardImageProcessing.h_v_lines(transformedLines)
        ...
        if h_lines is not None and v_lines is not None:
            intersection_points = goboardImageProcessing.line_intersections(h_lines, v_lines)
